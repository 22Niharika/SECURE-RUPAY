    ...
    const handleFaceLogin = async () => {
        if(isMounted){
            try{
                ...
                const un = username
                const res = await axios.post(`${BACKEND_URL}/face-login`, {
                    username,
                    image
                })
                if(res.data.status==="success"){
                    ...
                    if (window !== undefined){
                      window.localStorage.setItem("faceVerification", "true")
                    }
                    return router.push("/")
                  }
            }catch(err:any){
  ...
  />
        <div className=" inset-0 w-screen min-h-screen flex justify-center items-center z-20">
            <div className=" bg-white sm:px-8 shadow-lg rounded-lg sm:w-[25rem] w-5/6 flex items-center p-4 flex-col">
                   <div className="mb-4 text-black text-20xl">
                       Welcome {username}
                    </div>
